...
BoardWidget::BoardWidget(QWidget *parent)
    : QWidget{parent},trackPos(28, 28)//鼠标在棋盘上的位置
{
    ...
    for (int i = 0; i < BOARD_WIDTH - 1; i++)	//绘制棋盘格子
    {
        for (int j = 0; j < BOARD_HEIGHT - 1; j++)
        {
            painter.drawRect(QRect(START_POS + QPoint(i * CELL_SIZE.width(), j * CELL_SIZE.height()),
                                   CELL_SIZE));//在此位置画一个小格子
        }
    }
    //追踪框
    // 设置颜色，根据 nextPlayer 的值选择白色或黑色
    QColor color = (nextPlayer == WHITE_PLAYER) ? Qt::white : Qt::black;

    // 设置画笔颜色
    painter.setPen(color);

    // 定义一个包含12个QPoint的数组，表示多边形的顶点
    QPoint poses[12] = {
        trackPos + QPoint(0, 8),  // 第一个点，基于trackPos偏移 (0, 8)
        trackPos,                 // 第二个点，基于trackPos
        trackPos + QPoint(8, 0),  // 第三个点，基于trackPos偏移 (8, 0)
        ...
        for (int j = 0; j < BOARD_HEIGHT; j++)
        {
            if (board[i][j] != NO_PIECE)
            {
                ...
                painter.drawEllipse(START_POS.x() - CELL_SIZE.width()/2 + i*CELL_SIZE.width(),
                                    ...
                                    CELL_SIZE.width(), CELL_SIZE.height());//矩形尺寸
            }
        }
    }
...
void BoardWidget::mouseReleaseEvent(QMouseEvent *event)//获取鼠标单机输入下棋子
{
    if (!endGame)
    {
        ...
        int offsetX = x % CELL_SIZE.width();//超过左上角的点多少距离
...
void BoardWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    // 计算 x 和 y 坐标在单元格内的偏移量
    int offsetX = x % CELL_SIZE.width();
    int offsetY = y % CELL_SIZE.height();
    // 设置追踪位置，使其对齐到最近的单元格边界，并减去单元格宽度和高度的一半，以回到原始参考系
...
void BoardWidget::initBoard()
{
    ...
    update();
    //点击重新开始棋子消失 不用移动鼠标再更新
}

void BoardWidget::undo()//悔棋
{
    if (!endGame)
    {
        nextPlayer=(nextPlayer == WHITE_PLAYER) ? BLACK_PLAYER: WHITE_PLAYER;
        board[lastPos.x()][lastPos.y()] = NO_PIECE;
        lastPos = QPoint(-1, -1);
        update();

    }
}
void BoardWidget::downPiece(int x, int y)//落子
{
    // 检查 x 和 y 是否在棋盘范围内，并且该位置是否为空
...
bool BoardWidget::isFivePieceFrom(int x, int y)//五个棋子连成一线
{
...
bool BoardWidget::isVFivePieceFrom(int x, int y)
{
    ...
    for (int i = 1; i < 5; i++)
    {
        if (y + i >= BOARD_HEIGHT || board[x][y + i] != piece)//同一行
        {
            return false;
        }
    }
...
bool BoardWidget::isHFivePieceFrom(int x, int y)
{
    ...
    for (int i = 1; i < 5; i++)
    {
        if (x + i >= BOARD_WIDTH || board[x + i][y] != piece)//同一列
        {
            return false;
        }
    }
...
bool BoardWidget::isFSFivePieceFrom(int x, int y)
{
    ...
    for (int i = 1; i < 5; i++)
    {
        if (x + i >= BOARD_WIDTH || y - i < 0 || board[x + i][y - i] != piece)//同一列
        {
            return false;
        }
    }
...
bool BoardWidget::isBSFivePieceFrom(int x, int y)
{
    ...
    for (int i = 1; i < 5; i++)
    {
        if (x + i >= BOARD_WIDTH || y + i >= BOARD_HEIGHT || board[x + i][y + i] != piece)//同一对角线
        {
            return false;
        }
    }
...
void BoardWidget::setTrackPos(const QPoint &value)//记录追踪位置
{
    trackPos = value;
    update();
}
